...
/**
 * Streaming JDBC access for the transfer report. For a batch of input keys it emits one
 * {@link TransferReportRow} per transfer of every token belonging to the matching debit positions,
 * carrying the transfer detail ({@code TRANSFER_*}), the token/position context and {@code anag_*}
 * labels.
 *
...
public class TransferReportRepository {
    ...
    /**
     * Streams the transfer rows for the positions matching any key in the batch, issuing a single
     * set-based query.
     *
     ...
     * @param consumer receives every produced {@link TransferReportRow}
     * @return the number of rows produced
     */
    public long streamByKeys(CsvTemplate template, List<SearchInputRow> keys, AnalysisWindow window, Consumer<TransferReportRow> consumer) {
        MapSqlParameterSource params = new MapSqlParameterSource();
        String keyJoin = ReportKeyJoinSql.buildKeyJoin(template, schema, keys, params);
        if (keyJoin == null) {
            if (template == CsvTemplate.UNKNOWN) {
                log.warn("phase=REPORT_SKIP_BATCH report=transfer reason=unresolvable-template instanceId={} executionId={}",
    ...
    private TransferReportRow mapRow(ResultSet rs) throws SQLException {
        List<String> values = new ArrayList<>(TransferReportColumns.HEADERS.size());
        for (String column : TransferReportColumns.HEADERS) {
    ...
    private String buildBaseSelect(String schema) {
        ...
        return "SELECT"
            ...
            + " agg.transfer_number AS transfer_number,"
            + " t.amount AS amount,"
            + " psp.codice AS psp,"
            ...
            + "   FROM " + transfers + " tr2 WHERE tr2.fk_token = t.id"
            + " ) agg ON TRUE"
            + " LEFT JOIN LATERAL ("
